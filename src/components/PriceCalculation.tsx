...
const PriceCalculation: React.FC<PriceCalculationProps> = ({ formData }) => {
  ...
  const exportToPDF = async () => {
    ...
    doc.text('TALPOS SPECIFIKACIJA IR KAINOS SKAIČIAVIMAS', 20, 25);
    ...
    const specs = [
      ...
      `Vidinis paviršius: ${formData.innerSurface || 'Standard'}`,
      `Išorinis paviršius: ${formData.outerSurface || 'Standard'}`,
      `Orientacija: ${formData.orientation || 'Standartinė'}`,
      `Kojų skaičius: ${formData.legs || 0}`,
    ];
    
    specs.forEach((spec, index) => {
      doc.text(spec, 20, 65 + index * 6);
    });
    
    // Accessories if any
    if (formData.accessories && Object.values(formData.accessories).some(Boolean)) {
      let accessoryY = 65 + specs.length * 6 + 10;
      doc.setFont('helvetica', 'bold');
      doc.text('AKSESUARAI:', 20, accessoryY);
      ...
      const selectedAccessories = Object.entries(formData.accessories)
        ...
        .map(([key, _]) => {
          ...
          return `• ${accessoryNames[key] || key}`;
        });
      
      selectedAccessories.forEach((accessory, index) => {
        doc.text(accessory, 25, accessoryY + 8 + index * 6);
      });
    }
    ...
    // Technical drawings header
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('TECHNINIAI BRĖŽINIAI', 20, 25);
    
    // Try to capture 2D technical drawings
    try {
      // Look for technical drawing SVG elements
      const svgElements = document.querySelectorAll('svg[data-technical-drawing]');
      if (svgElements.length > 0) {
        ...
        for (let i = 0; i < Math.min(svgElements.length, 3); i++) {
          ...
          await new Promise((resolve) => {
            img.onload = () => {
              ...
              doc.addImage(imgData, 'PNG', 20, yPos, 160, 80);
              yPos += 90;
              resolve(void 0);
            };
        ...
        doc.setFont('helvetica', 'normal');
        doc.text('Techniniai brėžiniai galimi tik interaktyvioje versijoje', 20, 50);
      }
    } catch (error) {
      console.warn('Nepavyko įkelti techninių brėžinių į PDF:', error);
      doc.setFont('helvetica', 'normal');
      doc.text('Techniniai brėžiniai galimi tik interaktyvioje versijoje', 20, 50);
    }
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('3D VAIZDAS', 20, 25);
    
    // Try to capture 3D view
    try {
      ...
      if (canvas3D) {
        ...
        doc.setFont('helvetica', 'normal');
        doc.text('3D vaizdas galimas tik interaktyvioje versijoje', 20, 50);
      }
    } catch (error) {
      console.warn('Nepavyko įkelti 3D vaizdo į PDF:', error);
      doc.setFont('helvetica', 'normal');
      doc.text('3D vaizdas galimas tik interaktyvioje versijoje', 20, 50);
    }
    
    // Add new page for price calculation
    doc.addPage();
    
    // Price calculation header
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('KAINOS SKAIČIAVIMAS', 20, 25);
    ...
    // Save PDF
    doc.save(`talpos-specifikacija-${Date.now()}.pdf`);
  };
